...
#include <linux/syscalls.h> //gxp18
...
int avc_has_perm_flags(u32 ssid, u32 tsid, u16 tclass,
		       ...
		       unsigned flags) // GXP18 - Called by avc_has_perm() in avc.h
{
	struct av_decision avd;
	...
	perms = secclass_map[tclass-1].perms;
	security_sid_to_context(ssid, &scontext, &scontext_len);
	security_sid_to_context(tsid, &tcontext, &tcontext_len);


	// Monitor only open/read/write/append and execve - For sys calls append is an open with O_APPEND and a write

	i = 0;
	perm = 1;
	if (tmp_req != 0){

	   while (i < (sizeof(tmp_req) * 8)) { 
	      if ((perm & tmp_req) && perms[i] && (strcmp(perms[i],"read") == 0 || strcmp(perms[i],"write") == 0 || strcmp(perms[i],"open") == 0 || strcmp(perms[i],"execute") == 0 || strcmp(perms[i],"append") == 0) && strstr(secclass_map[tclass-1].name, "file") != NULL) { // filter out ops that do not refer to file	

		 struct pt_regs *regs = task_pt_regs(t);
		 int sn = syscall_get_nr(t, task_pt_regs(t));

		 // (CL) output format PROCESS PID SSID TSID TCLASS OPERATION SYSTEMCALL TARGET			
		 
		 if (sn == 322){ // __NR_openat
			fname = (char *) regs->ARM_r1;					
			printk(KERN_EMERG "(CL) %s %d %s %s %s %s openat %s\n", t->comm, t->pid, scontext, tcontext, secclass_map[tclass-1].name, perms[i], fname); //, args);
		 } else if (sn == 11){ // __NR_execve 
			//TODO - Find out why sometimes the fileN is ""		
			//int rc = strcmp(fileN, "");
			//if ( rc == 0 ) fileN = "fittizio";//gxp18
			mutex_lock(&file_mutex);
			fname = fileN;
			mutex_unlock(&file_mutex);
			printk(KERN_EMERG "(CL) %s %d %s %s %s %s execute %s\n", t->comm, t->pid, scontext, tcontext, secclass_map[tclass-1].name, perms[i], fname); // getname
		 } else if (sn == 3){ // __NR_read 
			file = fcheck_files(t->files, regs->ARM_r0);
			path = &file->f_path;
			path_get(path);
			...
			printk(KERN_EMERG "(CL) %s %d %s %s %s %s read %s\n", t->comm, t->pid, scontext, tcontext, secclass_map[tclass-1].name, perms[i], pathname);
		 } else if (sn == 4){ // __NR_write 
			file = fcheck_files(t->files, regs->ARM_r0);
			path = &file->f_path;
			path_get(path);
			tmp = (char *)__get_free_page(GFP_TEMPORARY);
			...
			pathname = d_path(path, tmp, PAGE_SIZE);
			printk(KERN_EMERG "(CL) %s %d %s %s %s %s write %s\n", t->comm, t->pid, scontext, tcontext, secclass_map[tclass-1].name, perms[i], pathname);
		 } else if (sn == 5){ 
			printk(KERN_EMERG "(CL) %s %d %s %s %s %s open %s\n", t->comm, t->pid, scontext, tcontext, secclass_map[tclass-1].name, perms[i], (char *) regs->ARM_r0);
		}
		tmp_req &= ~perm; 
	      }
	      i++;
